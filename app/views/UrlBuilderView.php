    ...
    <div class="container mt-5">
        ...
        <form action="?router=UrlBuilder/CreateUrl" method="post">
            ...
            <div class="d-flex justify-content-between align-items-center mt-3">
                ...
                <div class="mx-auto">
                    <form action="?router=UrlBuilder/loadUrls" method="post">
                        <button type="submit" class="btn btn-primary">Show URL History</button>
                    </form>
                </div>
                <div>
                    <form action="?router=UrlBuilder/exportUrls" method="post">
                        <button type="submit" class="btn btn-primary">Download URL History</button>
                    </form>
                </div>
            </div>
        </form>
        ...
        <?php if (isset($_SESSION['generated_url']) && $_SESSION['generated_url'] !== ''): ?>
            <div class="mt-3">
                <div class="alert alert-success w-100 text-break" role="alert">
                    ...
                    <?= htmlspecialchars($_SESSION['generated_url'], ENT_QUOTES, 'UTF-8'); ?>
                </div>
            </div>
        <?php endif; ?>
        <!-- Display URLs table when available -->
                        ...
                        <tbody>
                            <?php foreach ($all_urls as $url): ?>
                                <tr>
                                    ...
                                    <td><a href="<?= htmlspecialchars($url['link_url'], ENT_QUOTES, 'UTF-8'); ?>"
                                            target="_blank"><?= htmlspecialchars($url['link_url'], ENT_QUOTES, 'UTF-8'); ?></a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
